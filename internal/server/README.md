...
| 文件 | 说明 |
|------|------|
| `server.go` | HTTP 服务器：路由注册、并发控制（信号量）、去重（sync.Map）、优雅关闭 |
...
Server{
    ...
    inFlight: sync.Map,          // "group/project/42" -> bool，防止同一 MR 重复触发
...
POST /webhook/gitlab
  ...
  └─ 5. go s.triggerReview(event)  ← 异步执行
        │
        ├─ 去重: sync.Map.LoadOrStore("group/project/42")
        │       同一 MR 正在审查中 → 跳过，日志记录
        │       审查完成后 defer inFlight.Delete(key)
        │
...
| **去重** | `sync.Map` LoadOrStore | key 格式 `"path_with_namespace/iid"`，同一 MR 审查中不重复触发 |
